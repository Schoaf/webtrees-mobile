...
### Hinweis zum Server
"Geburtstage diese Woche" nutzt den bestehenden `Anniversaries`-Endpunkt des `webtreesand-api`-Moduls. Dessen Julian-Day-Berechnung (`->julianDay()` auf `CarbonImmutable`, nie eine echte Carbon-Methode) führte serverseitig zu einem 500-Fehler — behoben in `modules_v4/webtreesand-api/WebtreesAndApiModule.php` (nutzt jetzt `Registry::timestampFactory()->todayJulianDay()`, dieselbe Methode, die webtrees-Core intern für genau diesen Zweck verwendet). Geprüft: Der Fehler existiert **nicht** in webtrees selbst (dort gibt es diese Methode bereits) — daher kein Pull-Request nötig, nur der third-party-Modul-Fix hier im Repo.
